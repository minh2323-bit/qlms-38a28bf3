...
import {
  ...
  TextCursorInput, Link2, ToggleLeft, ArrowUpDown, Clock3, CheckCircle2,
  SlidersHorizontal, Check, Ban, MinusCircle, XCircle,
} from "lucide-react";
...
import {
  ...
  chapterTitle, lessonTitle, todayStr,
...
export function RejectModal({
  ...
  item: { content?: string; name?: string; proposer: string };
  onClose: () => void;
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          <div className="rounded-lg border bg-slate-50 p-3 text-sm space-y-1">
            <div><span className="text-slate-500">Tên học liệu: </span>
              <span className="font-semibold text-slate-800">{item.name ?? item.content}</span></div>
            <div><span className="text-slate-500">Người đề xuất: </span>
              <span className="font-semibold text-slate-800">{item.proposer}</span></div>
          </div>
          <div>
            <Label className="text-sm">Lý do từ chối <span className="text-slate-400 font-normal">(không bắt buộc)</span></Label>
            <Textarea className="mt-1 min-h-[110px]" value={reason}
        ...
        <DialogFooter>
          ...
          <Button className="bg-rose-600 hover:bg-rose-700" onClick={() => onConfirm(reason.trim())}>Từ chối</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
