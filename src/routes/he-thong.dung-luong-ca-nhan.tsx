...
export const Route = createFileRoute("/he-thong/dung-luong-ca-nhan")({
  validateSearch: (search: Record<string, unknown>) => ({
    role: search.role === "principal" ? ("principal" as const) : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Dung lượng bộ nhớ cá nhân | QLMS" },
      { name: "description", content: "Quản lý dung lượng cá nhân đã sử dụng trên LMS: danh sách tệp, ảnh bìa, nguồn sử dụng, tải về và xóa tệp để giải phóng bộ nhớ." },
      { property: "og:title", content: "Dung lượng bộ nhớ cá nhân" },
      { property: "og:description", content: "Theo dõi và dọn dẹp tệp, ảnh bìa cá nhân đã tải lên hệ thống LMS." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary" },
    ],
  }),
  component: PersonalStoragePage,
});
  ...
  return (
    <AppShell role={role}>
      <section className="space-y-4">
        <div>
          <h1 className="text-[19px] font-bold text-slate-800">Dung lượng bộ nhớ cá nhân</h1>
        ...
        <div className="flex items-center gap-6 border-b">
          {([
            ...
            <button
              ...
              className={`pb-2 text-[16px] font-bold transition ${
                tab === k
