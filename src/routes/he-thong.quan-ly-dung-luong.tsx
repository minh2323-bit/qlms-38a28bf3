...
export const Route = createFileRoute("/he-thong/quan-ly-dung-luong")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Theo dõi dung lượng bộ nhớ toàn trường và cá nhân: tỉ lệ đã dùng theo loại học liệu và bảng dung lượng sử dụng của từng giáo viên." },
      { property: "og:title", content: "Quản lý dung lượng lưu trữ" },
      { property: "og:description", content: "Dung lượng bộ nhớ toàn trường và cá nhân theo loại học liệu và theo giáo viên." },
...
const PERSONAL: { used: number; total: number; segs: Seg[] } = {
  used: 3.2,
  total: 10,
  segs: [
    { label: "Ảnh", gb: 1.12, color: "bg-rose-400" },
    { label: "Video", gb: 1.48, color: "bg-violet-400" },
    { label: "Tài liệu", gb: 0.4, color: "bg-sky-400" },
    { label: "Âm thanh", gb: 0.2, color: "bg-amber-400" },
  ],
};
          ...
          <div className="mt-1 h-4 w-full rounded-full bg-slate-100 overflow-hidden flex">
            {data.segs.map((s) => (
              <div
                key={s.label}
                className={s.color}
                style={{ width: `${(s.gb / data.total) * 100}%` }}
